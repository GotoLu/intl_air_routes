(function(global, factory) {
    ...
    var configOptions = {
        debug: false,
        wechatApiUrl: '//res.wx.qq.com/open/js/jweixin-1.0.0.js', // 微信 js
        wechatSignatureUrl: "//ccweixin.qunar.com/chat/getJsConfig.do", // 微信签名服务
        jsApiList: []
    };
    var name2KeyMap = {};
    var key2NameMap = {};
    ...
    function getSignature(callback) {
        ...
        xhr.open("POST", configOptions.wechatSignatureUrl, true);
    ...
    function adapter() {
        ...
        if(browser.wechat) {
            ...
            script.setAttribute('src', configOptions.wechatApiUrl);
            head.appendChild(script);
            script.onload = function() {
                if (wx) {
                    getSignature(function(data, err) {
                        var isError = false,
                            conf;
                        if (err || !data.ret) {
                            errorHandler( {
                        ...
                        conf = data.data;
                        conf.debug = configOptions.debug;
                        conf.jsApiList = configOptions.jsApiList;
                        wx.config(conf);
                        wx.ready(function (res) {
                            if (!isError) {
                                ...
                                cache = [];
                                readyCache.forEach(function(callback) {
    ...
    var API = {
        ...
        hy: {
            ...
            registerScreenshotShare: 'registerScreenshotShare',
            unregisterScreenshotShare: 'unregisterScreenshotShare',
            screenshotAndShare: 'screenshotAndShare'
        },
        ...
        h5: {
            ...
            login: function(cb, param){
                ...
                loginUrl = "//user.qunar.com/mobile/login.jsp";
                data = {
    ...
    configOptions.jsApiList = Object.keys(API.common).concat(Object.keys(API.share.wechat));
    ...
    QunarAPI = {
        ...
        ready: function(callback) {
            ...
            if(browser.qunar) {
                ...
                self.isReady = true;
                isFunction(callback) && callback.call(self);
            } else if(browser.wechat && isFunction(callback)) {
                if (self.isReady) {
                    callback.call(self);
                } else {
        ...
        config: function(opt) {
            ...
            for (var key in configOptions) {
                if (configOptions.hasOwnProperty(key) && opt.hasOwnProperty(key)) {
                    configOptions[key] = opt[key];
                }
            }
        },
